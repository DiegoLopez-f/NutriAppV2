...
interface PlanAlimento { refAlimento: string; cantidad: string; }
interface Macros { proteinas: number; carbohidratos: number; grasas: number; kcal: number; }
interface Comida { nombre: string; descripcion?: string; alimentos: PlanAlimento[]; macros?: Macros; }
interface VersionPlan { tipo: string; calorias: number; objetivo?: string; comidas: Comida[]; totales_diarios?: Macros; notas_tecnicas?: string[]; }
interface Plan { id: string; nombre: string; descripcion?: string; versiones: { [key: string]: VersionPlan }; asignadoA?: string; }
interface AlimentoBase { id: string; nombre: string; tipo: string; cantidadBase: number; proteina: number; carbohidratos: number; grasas: number; }
...
export default function PlanesPage() {
    ...
    if (!planActual || !versionData) return <div className="p-8 text-center text-gray-500">No hay planes disponibles.</div>;
            ...
            <div className="max-w-6xl mx-auto space-y-8">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div className="flex-1">
                        <span className="text-xs font-bold text-green-600 uppercase tracking-wide bg-green-50 px-2 py-1 rounded">Objetivo</span>
                        <p className="text-lg font-medium text-gray-800 mt-2">{versionData.objetivo || versionData.tipo}</p>
                    </div>
                        ...
                        {Object.keys(planActual.versiones).map(vKey => (
                            <button
                                ...
                                className={`px-6 py-2 rounded-lg text-sm font-bold transition-all ${
                                    versionKey === vKey ? 'bg-white text-green-600 shadow-md transform scale-105' : 'text-gray-500 hover:text-gray-700'
                                }`}
                            >
                ...
                {versionData.notas_tecnicas && versionData.notas_tecnicas.length > 0 && (
                    <div className="bg-amber-50 border border-amber-100 rounded-xl p-5">
                        <h3 className="text-amber-800 font-bold text-sm uppercase mb-3 flex items-center gap-2">
                            <ExclamationCircleIcon className="w-5 h-5" /> Notas Importantes
                        </h3>
                        <ul className="space-y-2">
                            {versionData.notas_tecnicas.map((nota, i) => (
                                <li key={i} className="text-amber-900 text-sm flex items-start"><span className="mr-2">•</span> {nota}</li>
                            ))}
                        </ul>
                    </div>
                )}
                                    ...
                                    {comida.alimentos.map((item, aIdx) => {
                                        ...
                                        if (!calculo) return (
                                            <tr key={aIdx} className="bg-red-50">
                                                <td className="px-6 py-4 text-red-600 font-medium flex gap-2 items-center">
                                                    <ExclamationCircleIcon className="w-4 h-4" /> {item.refAlimento}
                                                </td>
                                        ...
                                        return (
                                            <tr key={aIdx} className="hover:bg-gray-50/50">
                                                ...
                                                <td className="px-6 py-4 text-right font-mono text-gray-600">{item.cantidad}</td>
                                                <td className="px-2 py-4 text-center text-blue-700 bg-blue-50/30">{Math.round(calculo.p)}</td>
                                                <td className="px-2 py-4 text-center text-orange-700 bg-orange-50/30">{Math.round(calculo.c)}</td>
                                                <td className="px-2 py-4 text-center text-yellow-700 bg-yellow-50/30">{Math.round(calculo.g)}</td>
                                                <td className="px-2 py-4 text-center text-green-700 bg-green-50/30 font-bold">{Math.round(calculo.k)}</td>
                                            </tr>
                                        );
                                    })}
                                    </tbody>
...
const MacroCard = ({ label, value, unit, color, bg }: any) => (
    <div className={`p-4 rounded-xl ${bg} flex flex-col items-center justify-center text-center`}>
        ...
        <span className={`text-2xl font-extrabold ${color}`}>{Math.round(value)}{unit}</span>
    </div>
);