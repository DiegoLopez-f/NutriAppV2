...
interface VersionPlan { tipo: string; calorias: number; objetivo?: string; comidas: Comida[]; totales_diarios?: Macros; macros_total?: Macros; notas_tecnicas?: string[]; }
interface Plan { id: string; pacienteId: string; nombre: string; descripcion?: string; versiones: { [key: string]: VersionPlan }; }
...
export default function PlanesNutricionistaPage() {
    ...
    const calcularItem = (ref: string, cantidadSolicitada: string) => {
        ...
        return {
            p: alimento.proteina * factor,
            c: alimento.carbohidratos * factor,
            g: alimento.grasas * factor,
            k: (alimento.proteina * factor * 4) + (alimento.carbohidratos * factor * 4) + (alimento.grasas * factor * 9),
            nombreReal: alimento.nombre,
            tipo: alimento.tipo
        };
    };
                ...
                {planActual && versionData && (
                    <div className="space-y-6 animate-in fade-in">
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col md:flex-row justify-between items-start gap-6">
                            ...
                            >
                                {deletingId === planActual.id ? 'Eliminando...' : <><TrashIcon className="w-5 h-5" /> Eliminar Plan</>}
                            </button>
                        </div>
                        ...
                        {versionData.notas_tecnicas && versionData.notas_tecnicas.length > 0 && (
                            <div className="bg-amber-50 border border-amber-100 rounded-xl p-5">
                                <h3 className="text-amber-800 font-bold text-sm uppercase mb-3 flex items-center gap-2">
                                    <ExclamationCircleIcon className="w-5 h-5" /> Notas Técnicas
                                </h3>
                                            ...
                                            {comida.alimentos.map((item, aIdx) => {
                                                ...
                                                return (
                                                    <tr key={aIdx} className="hover:bg-gray-50/50">
                                                        ...
                                                        <td className="px-6 py-3 text-right font-mono text-gray-600">{item.cantidad}</td>
                                                        <td className="px-2 py-3 text-center text-blue-700 bg-blue-50/30">{Math.round(calculo.p)}</td>
                                                        <td className="px-2 py-3 text-center text-orange-700 bg-orange-50/30">{Math.round(calculo.c)}</td>
                                                        <td className="px-2 py-3 text-center text-yellow-700 bg-yellow-50/30">{Math.round(calculo.g)}</td>
